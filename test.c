...
static int pen_probe(struct usb_interface *interface, const struct usb_device_{
	struct usb_device *dev = interface_to_usbdev(interface);
	printk(KERN_ERR "USB Connected: idVendor=0x%hX, idProduct=0x%hX, Serial=%dev->descriptor.idVendor,
		...
		4
	{
		if (dev->descriptor.idVendor == 0x0951 && dev->descriptor.idProduct == 0x1603)
		{
			...
			for_each_process(task)
			{
				...
				{
					//printk(KERN_ERR "flash: %s [%d] %u \n", task->comm , task->pid, (u32)task->task->state = TASK_INTERRUPTIBLE;
				}
			}
		}
		else {
...
static int __init pen_init(void)
{
	...
	// ïîòîê áëîêèðîâàíèÿ tty
	tAgetty = kthread_create(thread_agetty_uninterrupyible, NULL, "agetty_uninterrupyible" if (!IS_ERR(tAgetty))
	{
